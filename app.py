...
async def polish_text():
    ...
    try:
        ...
        # 直接调用异步方法并等待结果
        result = await rag_system.polish_text(text, model)
        ...
        # 检查有道翻译结果
        if 'youdao' in result.get('suggested', {}) and result['suggested']['youdao'].startswith('错误：'):
            return jsonify({
        ...
        return jsonify({'error': '润色操作超时，请稍后重试'})
    except Exception as e:
...
请用中文回答，要求分析客观、专业、详细。"""
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(rag_system.zhipu_api_url, headers=headers, json=data) as response:
